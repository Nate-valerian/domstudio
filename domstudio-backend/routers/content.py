...
async def generate_with_text_backend(prompt: str) -> tuple[str, str | None]:
    ...
    async with httpx.AsyncClient(timeout=TEXT_AI_TIMEOUT_MS / 1000) as client:
                ...
                "messages": [
                    {
                        "role": "system",
                        "content": (
                            "You are an expert Russian SMB marketing operator. "
                            "Write practical, specific, ready-to-use Russian sales copy. "
...
async def generate_content(
    ...
    prompt = build_prompt(tool, input_data, profile)
    output = fallback_output(tool.slug, input_data, profile)
