...
class Payment(Base):
    ...
    original_amount_rub = Column(Float, nullable=True)
    discount_amount_rub = Column(Float, default=0)
    promo_code       = Column(String(32), ForeignKey("promo_codes.code"), nullable=True, index=True)
    promo_discount_percent = Column(Float, nullable=True)
...
class PromoCode(Base):
    ...
    owner_label        = Column(String(255), nullable=False)
    owner_user_id      = Column(UUID(as_uuid=True), ForeignKey("users.id"), nullable=True)
    parent_code        = Column(String(32), ForeignKey("promo_codes.code"), nullable=True, index=True)
    discount_percent   = Column(Float, default=15)
    commission_percent = Column(Float, default=15)
    ...
    parent = relationship("PromoCode", remote_side=[code])
...
class CommissionLedger(Base):
    ...
    code                  = Column(String(32), ForeignKey("promo_codes.code"), nullable=False, index=True)
    base_amount_rub       = Column(Float, nullable=False)
    commission_rate       = Column(Float, nullable=False)
    commission_amount_rub = Column(Float, nullable=False)
    payee                 = Column(String(255), nullable=False)
    payout_status         = Column(Enum(PayoutStatus, native_enum=False), default=PayoutStatus.pending)
    created_at            = Column(DateTime(timezone=True), server_default=func.now())
    updated_at            = Column(DateTime(timezone=True), onupdate=func.now())
